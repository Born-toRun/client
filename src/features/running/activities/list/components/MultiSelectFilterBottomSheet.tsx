...
/**
 * 다중 선택 필터 바텀시트 컴포넌트
 * 코스 필터 등 다중 선택이 필요한 경우 사용
 */
export default function MultiSelectFilterBottomSheet({
  open,
  onOpenChange,
  title,
  options,
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (!newOpen && !isApplied) {
      ...
      setTempSelected(selectedValues); // 원래 값으로 복원
    }
    onOpenChange();
  };
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <p className="text-n-500 body-sm mb-3">
            다중 선택이 가능합니다.
          </p>
